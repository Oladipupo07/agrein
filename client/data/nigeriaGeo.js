...
// Covers all 36 States + Federal Capital Territory (Abuja) — 774 LGAs in total.
...
const NIGERIAN_STATES_LGAS = {
  ...
  'Ekiti': [
    'Ado Ekiti', 'Efon', 'Ekamefa', 'Ekiti East', 'Ekiti South-West', 'Ekiti West',
...
const ALL_NIGERIAN_STATES = Object.keys(NIGERIAN_STATES_LGAS);

function getLgasForState(stateName) {
  if (!stateName) return [];
  const normalized = String(stateName).trim().toLowerCase().replace(/\s+state$/i, '');
  if (normalized === 'abuja' || normalized === 'fct' || normalized === 'federal capital territory' || normalized === 'fct - abuja') {
    return NIGERIAN_STATES_LGAS['FCT - Abuja'] || [];
  }
  const key = ALL_NIGERIAN_STATES.find(s => {
    const k = s.toLowerCase().replace(/\s+state$/i, '');
    return k === normalized || k.includes(normalized) || normalized.includes(k);
  });
  return key ? (NIGERIAN_STATES_LGAS[key] || []) : [];
}

// Global exposure
if (typeof window !== 'undefined') {
  window.NIGERIAN_STATES_LGAS = NIGERIAN_STATES_LGAS;
  window.ALL_NIGERIAN_STATES = ALL_NIGERIAN_STATES;
  window.getLgasForState = getLgasForState;
}

if (typeof module !== 'undefined' && module.exports) {
  module.exports = { NIGERIAN_STATES_LGAS, ALL_NIGERIAN_STATES, getLgasForState };
}
